...
# sh start.sh ${env} -- reload application @${env}

# examples:
# sh reload.sh pro -- use conf/nginx-pro.conf to reload lua_user-center
# sh reload.sh -- use conf/nginx-dev.conf to reload lua_user-center
...
/usr/local/openresty/nginx/sbin/nginx -s reload -p ${APP_ROOT_DIR} -c conf/nginx.conf
if [ $? != 0 ];then
    echo "启动失败，请检查"
    exit 1
fi
