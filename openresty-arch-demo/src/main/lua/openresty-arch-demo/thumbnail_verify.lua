...
if index and index > 0 and not file_util.exists(img_src_path) then
    temp_img_place = string.gsub(img_place, 'product/%d+/%d+/', '')
end
...
if index and index > 0 and not file_util.exists(img_src_path) then
    temp_img_place = string.gsub(img_place, 'product/%d+/%d+/', 'product/yt/')
end

if store_path_prefix then
    ngx_log(ngx_debug,"store_path_prefix",store_path_prefix)
    temp_img_src_path = store_path_prefix .. '/' .. temp_img_place .. '/' .. img_name
